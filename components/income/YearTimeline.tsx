...
import { accentFor, stackByDay } from "@/lib/income";
import type { IncomeItem, IncomeYear, Occurrence } from "@/lib/types";
import { OccurrenceTooltip } from "./OccurrenceTooltip";
...
export function YearTimeline({
  ...
  // day carrying three sources still fits inside the plot.
      ...
      >
        ...
        <g clipPath={`url(#${gradientId}-band)`}>
          {MONTH_NAMES.map((name, month) => {
            ...
            const share = derived.byMonth[month] / (derived.total || 1);
        ...
        {derived.occurrences.map((occurrence) => {
          ...
          const accent = accentFor(item?.accent ?? 0);
          ...
          return (
            ...
            >
              ...
              {/* A hairline of the panel colour keeps neighbouring bands legible
                  when two sources happen to share an accent. */}
              {shared && !capped ? (
                <line
                  x1={x - width / 2}
                  y1={top}
        ...
        {derived.occurrences.length === 0 ? (
          ...
          >
            Nothing lands in {year} yet — add income below.
          </text>
        ) : null}
      </svg>
